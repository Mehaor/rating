...
export const UserItem = ({item, position=0}) =>  <ListItem
    leftAvatar={
        <Avatar
          className='avatar'
          src={item.avatar}
          size={30}/>
      }>{position ? `${position}. ` : ''}{item.nickname}</ListItem>;

export const UserList = ({items}) => <List>
        {items.map((item, index) => { return <UserItem key={item._id} item={item} position={index + 1} /> })}
    </List>
